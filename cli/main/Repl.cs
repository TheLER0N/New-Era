...
partial class MainConsole
{
    static void Repl()
    {
        ...
        while (!StopRequested)
        {
            ...
            // ── Dispatcher v6.0: всегда ON, отключение запрещено ──
            if (lower == "dispatcher on")
            {
                DispatcherEnabled = true;
                WriteColored(ConsoleColor.Magenta,
                    "  ✔ Dispatcher: ON (always)" + NL);
                continue;
            }
            ...
            if (lower == "dispatcher off")
            {
                ...
                WriteColored(ConsoleColor.Yellow,
                    "  ⚠ Dispatcher нельзя отключить: ON" + NL);
                continue;
            }
            ...
            // ── Compress: всегда ON, отключение запрещено ──
            if (lower == "compress on")
            {
                CompressEnabled = true;
                WriteColored(ConsoleColor.Magenta,
                    "  ✔ Compress: ON (always)" + NL);
                continue;
            }
            ...
            if (lower == "compress off")
            {
                CompressEnabled = true;
                WriteColored(ConsoleColor.Yellow,
                    "  ⚠ Compress нельзя отключить: ON" + NL);
                continue;
            }

            // ── Extract: всегда ON, отключение запрещено ──
            if (lower == "extract on")
            {
                ExtractEnabled = true;
                WriteColored(ConsoleColor.Magenta,
                    "  ✔ Extract: ON (always)" + NL);
                continue;
            }
            ...
            if (lower == "extract off")
            {
                ExtractEnabled = true;
                WriteColored(ConsoleColor.Yellow,
                    "  ⚠ Extract нельзя отключить: ON" + NL);
                continue;
            }

            // ── Аркест-режим: всегда ON, отключение запрещено ──
            if (lower == "arc on")
            {
                ArcMode = true;
                WriteColored(ConsoleColor.Magenta,
