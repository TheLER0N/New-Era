// FileOps.cs — применение файлов, парсер, rollback, diff + Build Gate post-edit hook
// New Era v7.2+
using System;
...
partial class MainConsole
{
    static bool ApplyValidatedFiles(CodeWriterResult result, string projectPath, bool approved)
    {
        ...
        try
        {
            autoGate = BuildGate.ShouldRunAfterEdit(projectPath);
        }
        catch { }
        ...
        if (autoGate)
        {
            try
            {
                BuildGate.CreateRollbackPoint(projectPath, "edit");
            }
            catch { }
        }
        ...
        foreach (var op in result.Operations)
        {
            ...
            {
                WriteColored(ConsoleColor.Red, " \u2716 Путь вне проекта: " + op.Path + "\n");
                continue;
            }
            ...
            if (op.Action != null && op.Action.ToUpperInvariant() == "DELETE")
            {
                if (File.Exists(outPath))
                {
                    if (!approved)
                    {
                        ...
                        Console.Write(" \u2753 Удалить " + outPath + "? [y/N] ");
                        Console.ResetColor();

                        string c = Console.ReadLine();
                        if (c == null || c.Trim().ToLowerInvariant() != "y") continue;
                    }
            ...
            if (!approved)
            {
                ...
                if (c == null || c.Trim().ToLowerInvariant() != "y")
                    continue;
            }
        ...
        if (changed > 0 && autoGate)
        {
            try
            {
                BuildGate.RunPostApplyPipeline(projectPath);
            }
            catch { }
        }
    ...
    static CodeWriterResult ParseCodeWriterResponse(string raw)
    {
        ...
        foreach (string rawLine in lines)
        {
            ...
            if (trimmed.StartsWith("CONTENT:", StringComparison.OrdinalIgnoreCase) && currentPath != null)
            {
                inContent = true;
                continue;
            }
    ...
    static void SaveParsedBlock(CodeWriterResult result, string path, string action, string content)
    {
        ...
        result.Operations.Add(new CodeOperation
        {
            Path = path.Replace('\\', '/'),
            Action = string.IsNullOrEmpty(action) ? "MODIFY" : action,
    ...
    static void NormalizeSingleFileOperation(CodeWriterResult result, string filePath, string projectPath)
    {
        ...
        {
            result.Operations[0].Path = rel;
            if (string.IsNullOrEmpty(result.Operations[0].Action)) result.Operations[0].Action = "MODIFY";
            return;
        }
        ...
        for (int i = 0; i < result.Operations.Count; i++)
        {
            ...
            if (p.Equals(rel, StringComparison.OrdinalIgnoreCase) ||
        ...
        var first = result.Operations[0];
        result.Operations.Clear();
        result.Operations.Add(first);
        result.Operations[0].Path = rel;
    }
    ...
    static bool TryResolveSafeOutputPath(string baseDir, string relPath, out string fullPath)
    {
        ...
        if (string.IsNullOrWhiteSpace(relPath) || string.IsNullOrWhiteSpace(baseDir))
            return false;
    ...
    {
        string entry = "[" + DateTime.Now.ToString("dd.MM HH:mm:ss") + "] FILE: " + (file ?? "?") +
        ...
        lock (ChangeLog)
        {
            ...
            while (ChangeLog.Count > MaxChangeLogEntries)
    ...
    static void ShowDiff(string[] oldLines, int startLine, int endLine, string[] newLines)
    {
        lock (PrintLock)
        {
            ...
            Console.WriteLine("  \u256D\u2500 \u25B8 DIFF " + new string('\u2500', 30) + "\u256E");
            Console.ResetColor();

            for (int i = startLine; i <= endLine && i < oldLines.Length; i++)
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine("  \u2502 - " + oldLines[i]);
            }
...
class CodeOperation
{
    ...
    {
        get { return Action != null && Action.ToUpperInvariant() == "DELETE"; }
    }
}