...
static int GetJitter(int maxMs) { lock (RndLock) { return Rnd.Next(0, maxMs); } }
...
static bool LooksLikeServerBusy(string text)
{
    ...
    return t.Contains("high load") || t.Contains("overload") || t.Contains("too many requests") ||
           t.Contains("rate limit") || t.Contains("service unavailable") ||
           t.Contains("высокую нагрузку") || t.Contains("попробуйте позже") ||
           t.Contains("quota exceeded") || t.Contains("internal server error") ||
           t.Contains("http/1.1 503") || t.Contains("http/1.1 429");
}

static void MarkWafBlock() { Interlocked.Exchange(ref LastWafBlockTicks, DateTime.UtcNow.Ticks); }
...
static void EnforceWafCooldown()
{
    try {
        ...
        if (since.TotalSeconds < WafCooldownSeconds) {
            ...
            if (waitMs > 0) SleepInterruptible(waitMs);
        }
    } catch { }
}
...
static void SleepWaf(int attempt, string who)
{
    ...
    int delay = baseDelay + GetJitter(5000);
    WriteColored(ConsoleColor.Yellow, "    \u26A0 WAF (" + who + "): пауза " + (delay / 1000) + "с...\n");
    ...
    Action<string> parentSetter, string who, string dumpFile)
{
    ...
    for (int attempt = 1; attempt <= MaxAttempts; attempt++) {
        ...
        if (attempt > 1) {
            int delay = (lastEx != null && lastEx.Message.IndexOf("WAF", StringComparison.Ordinal) >= 0)
                ? WafBaseDelayMs + GetJitter(5000)
                : Math.Min(RetryDelayMs * attempt, 60000) + GetJitter(2000);
            WriteColored(ConsoleColor.Yellow, "  \u21BB " + who + ": попытка " + attempt + "/" + MaxAttempts + " через " + (delay / 1000) + "с...\n");
            SleepInterruptible(delay);
        }
        string raw;
        try {
            raw = PostMessageInternal(text, parentId, model, apiBase, timeoutMs, rwTimeoutMs, token, chatId, thinking);
        } catch (Exception ex) { lastEx = ex; continue; }

        if (!string.IsNullOrEmpty(dumpFile)) { try { File.WriteAllText(dumpFile, raw ?? "", new UTF8Encoding(false)); } catch { } }
...
static string PostRoleMessageWithRetry(string roleName, string systemPrompt, string userPrompt,
    string model, string apiBase, string token, string chatId, int timeoutMs, int rwTimeoutMs)
{
    if (string.IsNullOrEmpty(userPrompt)) throw new Exception(roleName + ": пустой запрос");
    string fullPrompt = string.IsNullOrEmpty(systemPrompt) ? userPrompt : systemPrompt + "\n" + userPrompt;
    bool isPrimaryChat = (token == Token && chatId == ChatId);
    bool thinking = isPrimaryChat || ShouldUseThinking(model);
    Action<string> setter = isPrimaryChat ? new Action<string>(SetPrimaryParent) : new Action<string>(SetAi2Parent);
    return PostAndParseWithRetry(fullPrompt, null, model, apiBase, timeoutMs, rwTimeoutMs, token, chatId, thinking, setter, roleName, isPrimaryChat ? DumpFile : null);
}

static string PostDispatchMessageWithRetry(string systemPrompt, string userPrompt)
{
    if (!IsAi2Configured()) throw new Exception("AI #2 не сконфигурирован");
    string fullPrompt = string.IsNullOrEmpty(systemPrompt) ? userPrompt : systemPrompt + "\n" + userPrompt;
...
static string PostMessageInternal(string text, string parentId, string model, string apiBase,
    int timeoutMs, int rwTimeoutMs, string token, string chatId, bool thinking)
{
    EnforceWafCooldown();
    ...
    if (!string.IsNullOrEmpty(chatId)) url += "?chat_id=" + Uri.EscapeDataString(chatId);
    ...
    for (int attempt = 0; attempt < InnerAttempts; attempt++) {
        ...
        if (attempt > 0) SleepInterruptible(RetryDelayMs * attempt + GetJitter(1000));
        ...
        string cookieVal = (!string.IsNullOrEmpty(CookieHeader) && token == Token) ? CookieHeader : ("token=" + token);
        try { var cc = new CookieContainer(); cc.SetCookies(new Uri(effBase), cookieVal); req.CookieContainer = cc; }
        catch { try { req.Headers[HttpRequestHeader.Cookie] = cookieVal; } catch { } }
        ...
        try { using (Stream rs = req.GetRequestStream()) { rs.Write(bodyBytes, 0, bodyBytes.Length); } }
        catch (Exception ex) { lastEx = ex; continue; }
        ...
        try {
            ...
            using (StreamReader reader = new StreamReader(stream, Encoding.UTF8)) { response = reader.ReadToEnd(); }
        } catch (WebException wex) {
            HttpWebResponse r = wex.Response as HttpWebResponse;
            int code = -1; string errBody = "";
            try { if (r != null) { code = (int)r.StatusCode; using (Stream es = r.GetResponseStream()) using (StreamReader er = new StreamReader(es, Encoding.UTF8)) { errBody = er.ReadToEnd(); } } }
            catch { } finally { if (r != null) { try { r.Close(); } catch { } } }

            string trimmedErr = (errBody ?? "").TrimStart();
            if (trimmedErr.StartsWith("<") || LooksLikeWafBlock(trimmedErr)) {
                MarkWafBlock(); lastEx = new Exception("HTTP " + code + " — WAF/капча");
                if (attempt + 1 < InnerAttempts) { SleepWaf(attempt, "HTTP"); continue; }
                throw new Exception("WAF блокирует.");
            }
            if (code == 401 || code == 403) throw new Exception("HTTP " + code + " — токен недействителен.");
        ...
        if (trimmed.Length == 0) { lastEx = new Exception("пустое тело"); if (attempt + 1 < InnerAttempts) continue; throw lastEx; }
        if (trimmed.StartsWith("<")) { if (trimmed.Contains("aliyun_waf")) { MarkWafBlock(); lastEx = new Exception("WAF (HTML)"); if (attempt + 1 < InnerAttempts) { SleepWaf(attempt, "HTTP"); continue; } } throw new Exception("HTML ответ."); }
        if (LooksLikeWafBlock(trimmed)) { MarkWafBlock(); lastEx = new Exception("WAF block"); if (attempt + 1 < InnerAttempts) { SleepWaf(attempt, "HTTP"); continue; } throw lastEx; }
        return response;
    }
...
static string ParseSseAnswerEx(string raw, Action<string> parentSetter)
{
    ...
    foreach (string rawLine in raw.Split(new[] { "\n" }, StringSplitOptions.None)) {
        ...
        try {
            ...
            if (obj.ContainsKey("status")) { string st = obj["status"] as string; if (st == "finished" || st == "completed") break; }
            string phase = FindJsonString(obj, "phase", 0);
            if (IsThinkingPhase(phase)) continue;
            string piece = ExtractDeltaContent(obj);
            if (!string.IsNullOrEmpty(piece)) AppendStreamPiece(answer, piece);
        } catch { }
    }
    if (!string.IsNullOrEmpty(newResponseId) && parentSetter != null) parentSetter(newResponseId);
...
static string ExtractDeltaContent(Dictionary<string, object> obj)
{
    ...
    foreach (object ch in choices) {
        ...
        if (choice == null || !choice.ContainsKey("delta")) continue;
        var delta = choice["delta"] as Dictionary<string, object>;
        if (delta != null && delta.ContainsKey("content")) { string c = delta["content"] as string; if (!string.IsNullOrEmpty(c)) sb.Append(c); }
    }
...
static void AppendStreamPiece(StringBuilder sb, string piece)
{
    ...
    if (curNoWs.Length > 0 && pieceNoWs.Length > 0 && pieceNoWs.IndexOf(curNoWs, StringComparison.Ordinal) >= 0 && piece.Length > current.Length) {
        sb.Length = 0; sb.Append(piece); return;
    }
    sb.Append(piece);
}
...
static string ParseFallback(string raw)
{
    ...
    if (trimmed.StartsWith("{")) {
        try {
            ...
            string phase = FindJsonString(obj, "phase", 0);
            if (!IsThinkingPhase(phase)) { string t = ExtractBestText(obj); if (!string.IsNullOrWhiteSpace(t)) return t.Trim(); }
        } catch { }
    }
    if (!trimmed.StartsWith("{") && !trimmed.StartsWith("<") && !trimmed.Contains("data:")) return trimmed.Trim();
    return null;
}
...
static string FindJsonString(object node, string key, int depth)
{
    ...
    if (dict != null) {
        if (dict.ContainsKey(key)) { string s = dict[key] as string; if (!string.IsNullOrEmpty(s)) return s; }
        foreach (var kv in dict) { string s = FindJsonString(kv.Value, key, depth + 1); if (!string.IsNullOrEmpty(s)) return s; }
        return null;
    }
    var arr = node as object[];
    if (arr != null) foreach (object el in arr) { string s = FindJsonString(el, key, depth + 1); if (!string.IsNullOrEmpty(s)) return s; }
    return null;
}
...
static string ExtractBestText(object node)
{
    ...
    foreach (string c in candidates) if (c != null && c.Length > bestLen) { best = c; bestLen = c.Length; }
    return best;
}
...
static void CollectBestCandidates(object node, List<string> candidates, int depth)
{
    ...
    var dict = node as Dictionary<string, object>;
    if (dict != null) {
        if (dict.ContainsKey("phase")) { string ph = dict["phase"] as string; if (IsThinkingPhase(ph)) return; }
        string[] keys = { "content", "text", "message", "delta", "choices", "content_list", "output", "result", "response", "answer" };
        foreach (string key in keys) { if (!dict.ContainsKey(key)) continue; CollectBestCandidates(dict[key], candidates, depth + 1); }
        return;
    }
    var arr = node as object[];
    if (arr != null) foreach (object el in arr) CollectBestCandidates(el, candidates, depth + 1);
}
...
static void PauseBetweenRoles() { if (!StopRequested) Thread.Sleep(RolePauseMs); }
}