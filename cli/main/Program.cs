...
partial class MainConsole
{
    ...
    static int Main(string[] args)
    {
        ...
        try { BuildGate.LoadBuildConfig(); } catch { }
    ...
    static void TryCreateDefaultConfig()
    {
        try
        {
            ...
            sb.AppendLine("# Build Gate");
            sb.AppendLine("BUILD_AFTER_EDIT=1");
            sb.AppendLine("AUTO_REPAIR=1");
            sb.AppendLine("VERIFY_MODE=smoke");
            sb.AppendLine("REPORT_DIR=.newera/reports");
            sb.AppendLine("MAX_DISK_ROLLBACKS=50");
    ...
    static void Repl()
    {
        while (!StopRequested)
        {
            ...
            if (lower == "help" || lower == "?") { DrawHelp(); BuildGate.DrawHelp(); continue; }

            if (lower == "status") { DrawStatus(); BuildGate.DrawBuildStatus(); continue; }
            ...
            // Build Gate
            if (lower.StartsWith("build") && (lower == "build" || lower.StartsWith("build "))) { BuildGate.HandleBuildCommand(cmd); continue; }
            if (lower.StartsWith("verify") && (lower == "verify" || lower.StartsWith("verify "))) { BuildGate.HandleVerifyCommand(cmd); continue; }
            if (lower.StartsWith("doctor") && (lower == "doctor" || lower.StartsWith("doctor "))) { Diagnostics.HandleDoctorCommand(cmd); continue; }
            if (lower.StartsWith("repair") && (lower == "repair" || lower.StartsWith("repair "))) { RepairLoop.HandleRepairCommand(cmd); continue; }
            if (lower.StartsWith("undo") && (lower == "undo" || lower.StartsWith("undo "))) { BuildGate.HandleUndoCommand(cmd); continue; }
