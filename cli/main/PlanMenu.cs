        ...
        string structure)
    {
        ...
        lock (PrintLock)
        {
            ...
            PlanMenuLine(
                "1",
                "Выполнить пошагово",
                "≈ " + steps.Count + " " + RequestsWord(steps.Count),
                ConsoleColor.Green,
                innerW);
        ...
        if (choice == "1")
        {
            ExecutePlan(steps, projectPath, originalTask, false);
        }
        else if (choice == "2")
        {
            ExecutePlan(steps, projectPath, originalTask, true);
        }
        else if (choice == "3")
        {
            if (DispatcherEnabled)
                ...
                ExecutePlanOneRequest(steps, projectPath, originalTask, structure);
        }
        else if (choice == "4")
        {
            SavePlanToFile(steps, projectPath, originalTask);
        }
        else
        {
            WriteColored(ConsoleColor.DarkGray,
                "  ◂ Отменено." + Environment.NewLine);
        }
    }

    static void PlanMenuLine(
        string num,
        string label,
        string hint,
        ConsoleColor color,
        ...
        string task)
    {
        try
        {
            ...
            WriteColored(ConsoleColor.Green,
                " ✔ Сохранён: " + planFile + Environment.NewLine);

            WriteColored(ConsoleColor.DarkGray,
                "    Повтор: /plan run" + Environment.NewLine);
        }
        catch (Exception ex)
        {
