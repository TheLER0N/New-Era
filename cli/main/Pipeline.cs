...
// New Era v7.2 · dispatch-вызовы через retry-слой (до 10 попыток)
using System;
...
partial class MainConsole
{
    const string PromptEnhance =
        ...
        "for a code-generation LLM. Keep original intent. Add specificity: which function/block to change, " +
        "what behavior to preserve, what coding style to follow. If the task is about editing code, add constraints: " +
        "do not rewrite unrelated code, preserve imports, maintain indentation and naming conventions. " +
        "Output ONLY the rewritten task. No preamble, no markdown.";

    const string PromptSelectFiles =
        "You are a file selector. Given a task, project tree and file contents preview, return ONLY JSON: " +
        "{\"files\": [\"relative/path1\", ...], \"actions\": {\"path\": \"READ|MODIFY|CREATE|DELETE\"}}. " +
        "3-15 files max. Select ONLY files relevant to the task. No markdown, no explanation.";

    const string PromptCompress =
        "You are a context compressor. Summarize the chat history into a short briefing (max 500 words). " +
        "Keep: key decisions, file names, errors, current task. Drop: code blocks, verbose explanations, greetings. " +
    ...
    static DispatchResult DispatchRequest(string userInput, string projectPath)
    {
        ...
        // 2. Enhance
        try {
            string enhanced = PostDispatchMessage(PromptEnhance, userInput);
    ...
    static List<FileSelection> SelectFilesViaAI2(string task, string projectPath)
    {
        ...
        string contentPreview = BuildSelectPreview(projectPath, 30000, 2000);
        ...
        string response = PostDispatchMessage(PromptSelectFiles, ub.ToString());
    ...
    static string CompressChatContext()
    {
        ...
        string response = PostDispatchMessage(PromptCompress, snapshot);
        if (string.IsNullOrWhiteSpace(response)) return null;
    ...
    // ══════════════════════════════════════════════
    //  EXTRACT CODE (P1: надёжный fallback)
    // ══════════════════════════════════════════════
    static CodeWriterResult ExtractCodeOrLocal(string rawResponse)
    {
        ...
        if (DispatcherEnabled && ExtractEnabled && IsAi2Configured()) {
            try {
                ...
                string response = PostDispatchMessage(PromptExtract, truncated);
    ...
    static bool ValidateOperationsViaAI2(CodeWriterResult result, out string details)
    {
        ...
        var sb = new StringBuilder();
        sb.Append("Validate these file operations. Check syntax, completeness, obvious logic errors.\n");
        ...
        try {
            string response = PostDispatchMessage("You are a code validator. Output PASS or FAIL and short errors.", sb.ToString());
            details = response;
    ...
    static string BuildPrimaryPrompt(DispatchResult dispatch, string projectPath)
    {
        ...
        if (dispatch.SelectedFiles != null && dispatch.SelectedFiles.Count > 0 && !string.IsNullOrWhiteSpace(projectPath)) {
            string payload = BuildSelectivePayload(dispatch.SelectedFiles, projectPath);
    ...
    static string BuildSelectivePayloadByPaths(List<string> fileList, string projectPath)
    {
        ...
        if (skipped > 0) sb.Append("\n// [selective: " + included + " loaded, " + skipped + " skipped]\n");

        return sb.ToString();
    }

    // ══════════════════════════════════════════════
    //  HTTP AI #2 — ТЕПЕРЬ ЧЕРЕЗ RETRY (до 10 попыток)
    // ══════════════════════════════════════════════
    static string PostDispatchMessage(string systemPrompt, string userPrompt)
    {
        return PostDispatchMessageWithRetry(systemPrompt, userPrompt);
    }
