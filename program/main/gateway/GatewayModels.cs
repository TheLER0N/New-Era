...
// Вопрос карточки «вопросы пользователю» (раунд 2): один request_user_input
// может нести массив таких вопросов, GUI рисует их ОДНОЙ карточкой.
class QuestionDto
{
...
// Раунд 3: запись индекса .leron/file_index.json — краткое описание файла
// плюс mtime/size для отметки свежести в промпте.
...
class AgentSession
{
    ...
    // Кэш прочитанных файлов: path → (mtime, length).
    // Повторное чтение неизменного файла вернёт заглушку вместо содержимого.
    public Dictionary<string, (DateTime MTime, long Length)> ReadCache { get; set; } = new(StringComparer.OrdinalIgnoreCase);
    // Файлы, изменённые ИИ в этой сессии: чтение своего файла вернёт заглушку.
    public HashSet<string> SelfModified { get; set; } = new(StringComparer.OrdinalIgnoreCase);
    // Раунд 4: агент уже получил контекст проекта (list_files / grep /
    // read_file / read_files). Пока флаг не взведён, мутации заблокированы.
    public bool HasContext { get; set; }
    // Раунд 5: текст последней ошибки проверки и счётчик одинаковых ошибок
    // подряд. 3 одинаковых подряд или конец шагов — карточка «продолжить чинить?».
...
internal sealed partial class GatewayState
{
    ...
    // Раунд 5: тестовые команды идут без подтверждения пользователя.
    ...
    // Чтение/разведка через shell (type/cat/Get-Content/Select-Object/dir/ls/tree…).
    // Такие команды вернут заглушку вместо вывода: чтение — только инструментами
    // read_file/read_files/grep, иначе ИИ тратит запросы на чтение по частям.
    ...
    // ── Парсинг: несколько JSON-блоков подряд ───────────────
    public static List<(string name, JsonObject args)> TryParseAllToolCalls(string text)
    {
        ...
        foreach (var candidate in ExtractJsonObjects(text))
        {
            try
            {
                ...
                if (name == "finish") break; // finish завершает цепочку
            }
            catch { }
        }
        return result;
    ...
    // Ремонт JSON от Qwen: живые переносы/табы внутри строк ломают парсер
    // (финиш с многострочным summary сгорал и стоил лишнего запроса).
