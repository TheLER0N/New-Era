...
internal sealed partial class GatewayState
{
    ...
    public async Task<string> ExecuteApprovedToolAsync(AgentSession s, PendingTool c)
    {
        ...
        // Проверка проекта больше НЕ запускается после каждой правки.
        // Она выполняется ОДИН раз в конце задачи (на finish) в RunBrowserAgentLoopAsync.
        return ToolResultPrompt(c.Name, result);
    }

    // Проверка проекта — вызывается только из RunBrowserAgentLoopAsync на finish.
    public async Task<string> RunProjectCheckAsync(AgentSession s, string trigger)
    {
        ...
        if (s.RepairAttempts > 2)
        {
            return
                ...
                "Ремонт остановлен — ждём ответа пользователя через request_user_input.";
        }
        return
            $"Автоматическая проверка '{cmd}' упала (попытка {s.RepairAttempts} из 2).\n" +
            $"Ошибка:\n{errorText}\n" +
            "Режим ремонта: прочитай ошибку, найди причину (read_file/read_files), исправь минимально через patch_file и вызови finish для повторной проверки.";
    }
}