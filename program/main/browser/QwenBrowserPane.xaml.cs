...
public partial class QwenBrowserPane : UserControl
{
    ...
    private async Task InjectCrtCss()
    {
        const string css = @"
:root {
  --bg-main: #02070c !important;
  --bg-sidebar: #02070c !important;
  --bg-panel: #02070c !important;
  --text-primary: #eaf6ff !important;
  --text-secondary: #6f96a8 !important;
  --accent: #00d9ff !important;
  --border: #12404f !important;
}
...
::-webkit-scrollbar-track { background: #02070c; }
::-webkit-scrollbar-thumb { background: #12404f; border-radius: 4px; }
::selection { background: #12404f; color: #eaf6ff; }
";
    ...
    private async void ConnectWebSocket()
    {
        ...
        try
        {
            ...
            Dispatcher.InvokeAsync(() => LinkDot.Fill = new SolidColorBrush((Color)ColorConverter.ConvertFromString("#00d9ff")));
            _ = ListenWebSocket();
        }
        catch
        {
    ...
    // Смена аккаунта Qwen: WebView2 хранит cookies (в т.ч. сессию Google),
    // поэтому «войти через Google» подставляло старый аккаунт с лимитами.
    // Полная очистка данных профиля сбрасывает и Qwen- , и Google-сессию —
    // после этого вход идёт как с чистого браузера, с выбором аккаунта.
    private async void OnSwitchAccountClick(object sender, RoutedEventArgs e)
    {
        try
        {
            if (WebView?.CoreWebView2 == null)
            {
                StatusText.Text = "⚠ WebView2 ещё не готов — нажми ещё раз через секунду.";
                return;
            }
            StatusText.Text = "Очищаю сессию Qwen (cookies, кэш)...";
            ConnectionStatus.Text = "Сессия сброшена — войди в нужный аккаунт и закрепи роль.";
        ...
        {
            StatusText.Text = $"Ошибка сброса аккаунта: {ex.Message}";
        }
    }
